...
* `mapped GestureType`：当前 class 映射到的动作类型，例如 `Start`、`Stop`、`Heart`、`Like` 或 `None`。
* `valid`：`GestureModel` 返回的调试字段。当前实时主程序主要依据 `GestureType` 触发动作；如果出现 `mapped=Start/Stop/Heart/Like` 但 `valid=false`，说明后续应检查 `GestureModel::parseOutput` 的 valid 赋值逻辑。
...
如果 `top1 class` 没有映射到 `Start`、`Stop`、`Heart`、`Like`，实时程序也不会触发动作。
