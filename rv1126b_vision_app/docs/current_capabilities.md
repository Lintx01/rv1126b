...
定时提醒配置位于 `AppConfig`：`drink_timer_reminder_enabled`、`drink_timer_interval_ms`、`drink_timer_repeat_ms`、`drink_timer_reset_on_drink_detected`、`drink_timer_confirm_ack_enabled`。默认首次提醒间隔为 5 分钟，重复提醒间隔为 5 分钟；调试时可在 `src/main.cpp` 中把 `config.drink_timer_interval_ms` 改成 `15000`，把 `config.drink_timer_repeat_ms` 改成 `10000`。
...
ST7789/LVGL 显示上，定时提醒合成为 `NeedRemind` 后显示 `DrinkRemindFace`；`DrinkDetected` 只用于清除 active timed reminder 并重新计时，不单独切换到喝水成功页。Start/Stop/Rock/Confirm 临时表情优先级最高，临时表情期间不会被基础状态强行覆盖；基础状态显示优先级为 `BadPostureFace` > `DrinkRemindFace` > `SleepFace` > `NormalFace`，坐姿异常会优先于饮水提醒展示。

用户通过 Confirm/OK 或 Rock 手势表示“知道了”时，会触发坐姿提醒屏幕缓冲：默认 `posture_ack_silence_ms = 30000`，30 秒内 ST7789 不再抢回 `BadPostureFace`，但 AI/Web/MQTT 仍保留真实的 `bad_alert` 状态。`BadPostureFace` 和 `DrinkRemindFace` 均使用 LVGL 动态页持续刷新，便于比赛展示。
...
基础状态表情选择时，喝水成功不单独展示；坐姿异常比饮水提醒更紧急；只有没有坐姿异常和饮水提醒时，待机状态才显示 SleepFace。
...
4. `[Perf]` 日志可作为 AI 耗时量化指标：`AI总耗时平均_ms` 表示一次 AI 调度从开始处理到发布结果的平均耗时；`AI感知链路平均_ms` 表示姿态/水杯感知链路耗时；`AI预处理平均_ms` 表示图像预处理耗时；`手势推理平均_ms`、`姿态推理平均_ms`、`水杯推理平均_ms`、`旧模型推理平均_ms` 表示对应模型推理平均耗时。每个耗时项同时输出 `最小_ms` 和 `最大_ms`，用于展示最小/最大耗时。
5. ST7789 当前待机页可用，Start/Stop/Confirm/Rock 四类手势会推送对应业务表情；`--display-only` 仍只显示待机时钟。
6. 北京时间显示策略需要确认，避免系统时间和代码偏移重复。当前配置里有 `display_timezone_offset_minutes = 480`。
7. 当前代码版本多，命令和文档容易混乱，需要以本文件为当前版本准则。
8. 等比补边 功能后续准备移除或整理，不作为当前比赛主线。
9. `RV_FORCE_AI_RUNNING=1` 会让主程序初始进入 Running 状态；默认不设置时仍然从 Idle 开始，等待手势启动。
10. `已移除的预处理几何环境变量` 本次 grep 未在 `src/`、`include/`、`tools/`、`run_demo.sh` 中发现，当前不应作为主线运行参数。
11. MQTT 代码存在，但当前主程序运行配置默认关闭；如果要作为比赛状态数据链路，需要单独确认 broker、编译选项和运行配置。
...
```bash
grep -E "[AI].*gesture|[AI].*pose|[AI].*cup|AI总耗时|AI感知链路|AI预处理|手势推理|姿态推理|水杯推理|旧模型推理" logs/run_model.log
```
